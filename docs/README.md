...
- `03-customer-miniapp.md`：当前客户端总纲
- `03a-customer-appointment.md`：第 1～10 节为已确认预约；第 11 节为容量 / 改期提案（待确认，不可直接实施）
...
- 启动封面广告关闭、无填充、过期或加载失败时必须恢复原入口目标（无明确目标时进入首页）；内部页面跳转不重复弹启动广告。
...
- 宠物交友默认自动轮播并支持手动滑动；每屏 3 只、一轮最多 12 只，本轮真实宠物至少 4 只且包含本人主宠物时自己的主宠物固定第 4 位；不足 4 只按实际数量展示；真正可邀请卡使用底部浅色“可发送邀请”标签。
