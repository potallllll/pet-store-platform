...
- 店员端讨论材料见 [04-staff-miniapp.md](04-staff-miniapp.md)；工作台分组与组内顺序已在该文件§3.2确认；继续讨论其余工作台细节与全部菜单，具体提案状态和本端待决索引由该文件维护。
- DeepSeek执行准备见 [执行标准](execution/deepseek-execution-standard.md)；执行准备决策的唯一明细为该文§1的DEC-EXEC-01/02。
- 店员新增讨论决策的唯一明细为04文档§7的DEC-STAFF-01～08；此处不重复问题内容。
- 新增标准、提示词与追踪表不改变本文件顶部当前阶段，不把客户端基线升级为frozen，不代表店员端已经确认或代码验收通过。
...
| `03h-customer-risk-feedback.md` | 已确认投诉状态机、7天回应、3天补证、单审核员、长期公开、食品2天/3条风险规则、宠物当前品牌风险提醒、问题热度必须展示、批量审核基线、双方门店授权且顾客分项同意后手机号或微信号任一匹配可查顾客共享风险提醒 | 集中反馈续期、重复投诉、跨店顾客风险、宠物安全共享、证据权重、未入驻门店、食品多主体回应、最终UI、问题热度算法 | `DEC-COMPLAINT-07`、`08`、`DEC-RISK-01`、`DEC-PETSAFETY-01`、`DEC-EVIDENCE-01`、`DEC-STORE-CLAIM-01`、`DEC-FOOD-RESP-01`、`DEC-RISK-UI-01`、`DEC-FOOD-HEAT-01` |
...
| `10a-customer-ui-design-system.md` | 通用 Token 与其他已确认 UI | 消费争议模块最终视觉 | `DEC-RISK-UI-01` |
| `10b-customer-homepage-ui-spec.md` | 首页已确认结构与其他模块 | §7 消费争议精确布局 | `DEC-RISK-UI-01` |
...
| DEC-RISK-01 | 双方门店授权及手机号／微信号任一匹配方向已确认；顾客端服务履约记录名称与分项默认不勾选已确认；授权操作与撤回、标识安全、保存、申诉与业务影响待决 | 03h §10.1；阶段08/合规 |
...
| DEC-FOOD-RESP-01 | 已确认客服邮件联系厂家、管理员在平台Web后台录入回信；正式回应判定、多主体责任及期限映射待决 | 03h §11 |
| DEC-RISK-UI-01 | 首页消费争议卡片、距离、筛选与详情精确视觉 | 03h §11；10a/10b |
| DEC-FOOD-HEAT-01 | 食品问题热度的数值范围、公式、权重、更新频率与最终显示形态 | 03h §8.6；阶段06/11 |
